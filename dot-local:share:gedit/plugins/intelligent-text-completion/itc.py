                    ...
                    if not cursor.ends_line():
                        if next_char == add_char:
                            if check_char != add_char:
                                ...
                                preceding_check_chars = len(re.findall(
                                    '\%s' % check_char, preceding_line))
                                preceding_add_chars = len(re.findall(
                                    '\%s' % add_char, preceding_line))
                    ...
                    if prev_char == indent_trigger:
                        if line_after:
                            ...
                            if ending_char != '' and ending_char in line_after:
                                ...
                                end.get_line_offset() + ending_pos)
                            ending_text = doc.get_text(
                                cursor, end, False).strip()
                            doc.delete(cursor, end)

                            add_middle = typed_char + \
                                whitespace + get_tab_string(view)
...
class IntelligentTextCompletionOptions(object):
    ...
    def __init__(self):
        ...
        # load settings
        self.closeBracketsAndQuotes = self._load_setting(
            "closeBracketsAndQuotes")
        self.completeXML = self._load_setting("completeXML")
        self.detectLists = self._load_setting("detectLists")
        self.autoindentAfterFunctionOrList = self._load_setting(
            "autoindentAfterFunctionOrList")
    ...
    def _on_check_button_toggled(self, *args):
        ...
        self._save_setting("closeBracketsAndQuotes",
                           self.closeBracketsAndQuotes)
        self._save_setting("completeXML", self.completeXML)
        self._save_setting("detectLists", self.detectLists)
        self._save_setting("autoindentAfterFunctionOrList",
                           self.autoindentAfterFunctionOrList)
