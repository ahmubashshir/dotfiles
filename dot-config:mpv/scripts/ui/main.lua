...
function set_osd(res_x, res_y, text)
    if state.osd.res_x == res_x and state.osd.res_y == res_y and state.osd.data ==
...
function mouse_hit(element) return mouse_hit_coords(get_element_hitbox(element)) end
...
function countone(val)
    if not (user_opts.iamaprogrammer) then val = val + 1 end
    return val
end
...
function ass_append_alpha(ass, alpha, modifier)
    ...
    ass:append(string.format('{\\1a&H%X&\\2a&H%X&\\3a&H%X&\\4a&H%X&}', ar[1],
                             ar[2], ar[3], ar[4]))
end
...
function get_tracklist(type)
    ...
    if #tracks_osc[type] == 0 then
        ...
        for n = 1, #tracks_osc[type] do
            ...
            local lang, title, selected = 'unknown', '', (track.id ==
                                              tonumber(mp.get_property(type)))
            if not (track.lang == nil) then lang = track.lang end
            if not (track.title == nil) then title = track.title end
            msg = msg .. '\n' ..
                      icons[(selected and '' or 'not-') .. 'selected'] .. ' ' ..
                      n .. ': [' .. lang .. '] ' .. title
        end
    end
    return msg
                    ...
                    if (marker >= element.slider.min.value) and
                        ...
                        if (slider_lo.gap > 5) then -- draw triangles
                            ...
                            if (slider_lo.nibbles_bottom) then
                                static_ass:move_to(s - 3, elem_geo.h -
                                                       slider_lo.gap + 5)
                                static_ass:line_to(s,
                                                   elem_geo.h - slider_lo.gap +
                                                       1)
                                static_ass:line_to(s + 3, elem_geo.h -
                                                       slider_lo.gap + 5)
                            end
                        else -- draw 2x1px nibbles
                            -- top
                            if (slider_lo.nibbles_top) then
                                static_ass:rect_cw(s - 1, 0, s + 1,
                                                   slider_lo.gap);
                            end
                            -- bottom
                            if (slider_lo.nibbles_bottom) then
    ...
    for n = 1, #elements do
        ...
        if element.eventresponder and (state.active_element == n) then
            ...
            if mouse_hit(element) then
                ...
                if (element.styledown) then
                    style_ass:append(styles.elementDown)
                end
                if (element.softrepeat) and
                    (state.mouse_down_counter >= 15 and state.mouse_down_counter %
                        5 == 0) then
        ...
        if (element.type == 'slider') then
            ...
            if pos then
                ...
                elem_ass:rect_cw(0, slider_lo.gap, xp,
                                 elem_geo.h - slider_lo.gap)
            end
            ...
            if seekRanges then
                ...
                for _, range in pairs(seekRanges) do
                    ...
                    elem_ass:rect_cw(pstart - rh, slider_lo.gap, pend + rh,
                                     elem_geo.h - slider_lo.gap)
                end
            end
        ...
        elseif (element.type == 'button') then
            ...
            local charcount = (buttontext:len() +
                                  select(2, buttontext:gsub('[^\128-\193]', '')) *
                                  2) / 3
...
function get_playlist()
    ...
    for i, v in ipairs(limlist) do
        ...
        message = string.format('%s %s %s\n', message,
                                icons[(v.current and '' or 'not-') .. 'selected'],
                                title)
    end
    return message
end
...
function render_message(ass)
    if state.message_hide_timer and state.message_hide_timer:is_enabled() and
        ...
        fontsize = fontsize * counterscale /
                       math.max(0.65 + math.min(lines / maxlines, 1), 1)
        outline = outline * counterscale /
                      math.max(0.75 + math.min(lines / maxlines, 1) / 2, 1)
...
function add_layout(name)
    if not (elements[name] == nil) then
        ...
        msg.error('Can\'t add_layout to element \'' .. name ..
                      '\', doesn\'t exist.')
    end
end
...
function window_controls()
    ...
    add_area('window-controls', get_hitbox_coords(controlbox_left, wc_geo.y,
                                                  wc_geo.an, controlbox_w,
                                                  wc_geo.h))
    ...
    if state.maximized or state.fullscreen then
        ne.content = icons['button']['unmaximize']
    else
        ne.content = icons['button']['maximize']
    end
    ne.eventresponder['mbtn_left_up'] = function()
...
layouts = function()
    ...
    lo.style = styles.Ctrl3

    lo = add_layout('volume')
    lo.geometry = {x = 87, y = refY - 40, an = 5, w = 30, h = 24}
    lo.style = styles.Ctrl2
    ...
    lo = add_layout('skip-op-ed')
    lo.geometry = {x = osc_geo.w - 137, y = refY - 40, an = 5, w = 30, h = 24}
    lo.style = styles.Ctrl2
    ...
    lo = add_layout('title')
    lo.geometry = geo
    lo.style = string.format('%s{\\clip(%f,%f,%f,%f)}', styles.Title, geo.x,
                             geo.y - geo.h, geo.x + geo.w, geo.y)
    lo.alpha[3] = 0
end

-- Validate string type user options
function validate_user_opts()
    if user_opts.windowcontrols ~= 'auto' and user_opts.windowcontrols ~= 'yes' and
        user_opts.windowcontrols ~= 'no' then
        msg.warn('windowcontrols cannot be \'' .. user_opts.windowcontrols ..
                     '\'. Ignoring.')
...
function osc_init()
    ...
    ne.content = icons['button']['previous']
    ...
    ne.content = function()
        if mp.get_property('pause') == 'yes' then
            return icons['button']['play']
        else
            return icons['button']['pause']
        end
    end
    ne.eventresponder['mbtn_left_up'] = function()
    ...
    ne.softrepeat = true
    ne.content = icons['button']['skip-backward']
    ...
    ne.softrepeat = true
    ne.content = icons['button']['skip-forward']
    ne.eventresponder['mbtn_left_down'] =
        multi_handler(have_ch, function()
            mp.commandv("add", "chapter", 1)
        end, function() mp.commandv('seek', 10, 'relative', 'keyframes') end)
    ...
    ne.tooltip_style = styles.Tooltip
    ne.softrepeat = true
    ne.content = function()
        vol = state.volume
        return icons['button']['volume-' .. (not state.mute and
                   (vol > 100 and 'max' or vol > 70 and 'full' or vol > 40 and
                       'half' or vol > 40 and 'half' or vol > 0 and 'low') or
                   'mute')]
    end
    ne.tooltipF = function() return state.volume .. '%' end
    ne.eventresponder['wheel_up_press'] = function()
        mp.commandv('add', 'volume', 1)
    end
    ...
    -- skip op/ed
    ne = new_element('skip-op-ed', 'button')

    ne.softrepeat = true
    ne.content = icons['button']['skip-intro']
    ne.tooltip_style = styles.Tooltip
    ne.tooltipF = "L-R: Skip-Unskip OP/ED"
    ne.eventresponder['mbtn_left_down'] = function()
    ...
    ne.content = icons['subtitles']
    ne.tooltip_style = styles.Tooltip
    ne.tooltipF = function()
        local msg = texts.off
        if not (get_track('sub') == 0) then
            msg = (texts.subtitle .. ' [' .. get_track('sub') .. ' ∕ ' ..
                      #tracks_osc.sub .. '] ')
    ...
    ne.content = function()
        return icons['button'][(state.fullscreen and 'exit-' or '') ..
                   'fullscreen']
    end
    ne.visible = (osc_param.playresx >= 540)
    ne.eventresponder['mbtn_left_up'] = function()
...
function render()
    ...
    if not (state.mp_screen_sizeX == current_screen_sizeX and
        state.mp_screen_sizeY == current_screen_sizeY) then
    ...
    if state.initREQ then
        ...
        if (state.last_mouseX == nil or state.last_mouseY == nil) and
            not (mouseX == nil or mouseY == nil) then
    ...
    if not (state.anitype == nil) then
        ...
        if (now < state.anistart + (user_opts.fadeduration / 1000)) then
            ...
            if (state.anitype == 'in') then -- fade in
                osc_visible(true)
                state.animation = scale_value(state.anistart, (state.anistart +
                                                  (user_opts.fadeduration / 1000)),
                                              255, 0, now)
    ...
    -- submit
    set_osd(osc_param.playresy * osc_param.display_aspect, osc_param.playresy,
            ass.text)
end
        ...
        if (user_opts.minmousemove == 0) or
            (not ((state.last_mouseX == nil) or (state.last_mouseY == nil)) and
                ((math.abs(mouseX - state.last_mouseX) >= user_opts.minmousemove) or
                    (math.abs(mouseY - state.last_mouseY) >=
                        user_opts.minmousemove))) then show_osc() end
        state.last_mouseX, state.last_mouseY = mouseX, mouseY
...
function tick()
    ...
    if state.anitype ~= nil then request_tick() end
end
...
mp.register_script_message('osc-playlist',
                           function(dur) show_message(get_playlist(), dur) end)
...
mp.observe_property('volume', 'number',
                    function(name, val) state.volume = val end)
...
mp.set_key_bindings({
    ...
    {'mbtn_left_dbl', 'ignore'}, {'mbtn_right_dbl', 'ignore'}
}, 'input', 'force')
mp.enable_key_bindings('input')
