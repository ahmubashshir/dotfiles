...
if command -pv systemct && systemctl is-system-running -q; then
	if systemctl is-active -q docker.socket;then
		export DOCKER_HOST="unix:///var$(systemctl cat docker.socket|awk -F = '/^ListenStream/{print $2;exit}')"
	elif systemctl --user is-active -q podman.socket;then
